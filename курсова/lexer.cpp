...
unsigned int GetTokens(FILE* F, Token TokenTable[], FILE* errFile)
{
	...
	while (1)
	{
		...
		case Letter:
		{
			...
			else if (!strcmp(buf, "scan"))		temp_type = Input;
			else if (!strcmp(buf, "print"))	temp_type = Output;

			else if (!strcmp(buf, "add"))		temp_type = Add;
			else if (!strcmp(buf, "sub"))		temp_type = Sub;

			else if (!strcmp(buf, "eg"))		temp_type = Equality;
			else if (!strcmp(buf, "ne"))		temp_type = NotEquality;

			else if (!strcmp(buf, "not"))		temp_type = Not;
			else if (!strcmp(buf, "and"))		temp_type = And;
			else if (!strcmp(buf, "or"))		temp_type = Or;
